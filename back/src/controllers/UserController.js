...
export const users_get_list = async (req, res) => {
  const users = await UserModel.query().withGraphFetched("[posts, comments, role]").where("deleted_at", null)
  res.send(users)
}
...
export const users_get_description = async (req, res) => {
  ...
  */
  ...
  const user = await UserModel.query().findById(userId).where("deleted_at", null)
